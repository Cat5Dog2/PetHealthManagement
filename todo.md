# うちの子健康カルテ開発 TODO（実行計画）

> 対象：うちの子健康カルテ（ASP.NET Core MVC + Identity / EF Core / SQL Server、開発：LocalDB、本番：Azure SQL）
...
### 共通ルール
- **正式アプリ名**：ユーザー向け表示名は **「うちの子健康カルテ」** を正とする
- **UI方針**：スマホ利用を主対象に、モバイルファーストのレスポンシブUIを正とする
...
### 1.5 完了条件/成果物
- [x] ローカル起動→ログイン→基本画面遷移が一周できる（エラーページ含む）
...
### 2.2 マイグレーション/初期データ
- [x] 初回 Migration 作成・適用
- [x] 開発用 Seed（Species、Adminユーザー/ロール付与手順）
- [x] 本番向け Migration 運用手順（適用順、ロールバック方針）

### 2.3 完了条件/成果物
- [x] 新規環境で「マイグレーション適用→Seed→ログイン→初期表示」まで手順化されている
...
- [x] **同時アップロード/連投対策**：ASP.NET Core Rate Limiting（IP/ユーザー）を導入（まずは緩めでOK）
- [x] 画像アップロード失敗時のエラーメッセージ（ユーザー向け/ログ向け）を整理
...
### 3.5 完了条件/成果物
- [x] 画像アップロード→保存→認可付き配信→削除 が一連で動作（異常系：偽装/超過/上限も含む）
- [x] 画像関連のログ方針（削除失敗/拒否理由/例外）が決まっている
...
### 6.4 完了条件/成果物
- [x] CRUD + 画像複数 + 制限（10枚/100MB/2MB/解像度）が確認できる
...
- [x] DBテスト戦略（LocalDB/テストDB、トランザクション、データリセット）
- [x] ストレージ（画像）テスト戦略（テスト用StorageRoot、クリーンアップ）

### 11.2 重要シナリオ
- [x] 認証：未ログイン→保護URL→Login→returnUrl復帰
- [x] 存在秘匿：他人の非公開Pet/健康情報/画像が404
- [x] 画像：拡張子偽装、2MB超、解像度超、合計100MB超、最大10枚超
...
### 11.4 テストケース反映
- [x] 画面単位テストケース表の項目を、結合テスト/E2Eに落とし込み

### 11.5 完了条件/成果物
- [x] CIで主要シナリオが自動で回り、回帰を検知できる（最低限：認証/秘匿/画像制限）
...
- [x] 画像バックアップ/ライフサイクルの手順を「デプロイ後運用」に紐づけて文書化
- 決定事項：画像バックアップは **`Storage__RootPath` 配下全体**を対象にし、日次バックアップ + 画像処理変更前/削除系保守前のオンデマンドバックアップを正とする。復元は隔離先で確認後、必要な `images/...` のみ production に戻す。`tmp/` の残骸は週次点検で掃除する。
- [x] 画像削除失敗の再試行（リトライキュー/定期バッチ等）を運用タスクとして位置づけ
- 決定事項：初期リリースでは retry queue / 定期バッチはまだ入れず、**Application Insights の `Failed to delete image file` ログを起点にした運用再試行**を正とする。参照切れ確認後に手動削除し、失敗が継続する場合は別タスクで自動再試行を実装する。

### 12.5 完了条件/成果物
- [x] 「デプロイ→マイグレーション→スモーク→監視→ロールバック」まで1本の手順書になっている
- 決定事項：Azure 運用手順の正本は `README.md` とし、デプロイ・Migration・smoke・監視・rollback・画像運用 Runbook を同じ文書で更新していく。

---

## 追加タスク（初期リリース対象外 / 必要に応じて）
- 決定事項：このセクションの未チェック項目は第2段階以降または任意改善の候補とし、初期リリースの未完了要件としては扱わない。
- [x] **第1段階UI刷新**：正式アプリ名「うちの子健康カルテ」への表示統一、共通レイアウトのモバイルファースト化、既存機能（Home/MyPage/Pets/HealthLogs/ScheduleItems/Visits/Account）のスマホ向けカード/フォームUI化
- [x] **第1段階UIの範囲整理**：健康分析、月間カレンダー、通知設定、データバックアップ、利用規約、アプリについて等は新機能扱いとし、第1段階では未実装リンクやダミー画面を追加しない
- [x] **ペットプロフィール項目の仕様反映**：`Sex` / `BirthDate` / `AdoptedDate` は正式仕様に残し、Entity / Migration / ViewModel / View / テストへ反映済み
- [x] **モバイルUI/UX改善（2026-07 実施）**
- 決定事項：ボトムナビは **ホーム(/MyPage) / ペット(/Pets) / 記録(/HealthLogs/Record) / 設定(/Account/EditProfile)** の4項目（アイコン+ラベル、`aria-current` で現在地表示、768px 以上では非表示で上部ナビに一本化）を正とする。
- 決定事項：`GET /HealthLogs/Record` を追加し、日次記録の入口とする（ペット0匹→ `/Pets/Create`、1匹→作成へ302、2匹以上→ペット選択画面）。
- 決定事項：ログイン済みユーザーの `/` アクセスは `/MyPage` へ 302 リダイレクトする（トップは未ログイン専用）。
- 決定事項：POST 成功後は TempData（キー `StatusMessage`、文言は `Infrastructure/StatusMessages.cs`）による共通ステータスアラートで結果を表示する。
- 決定事項：削除系フォームは全画面で `confirm` ダイアログを必須にする。アカウント削除の導線は MyPage からプロフィール編集画面内の危険領域へ移動する。
- 決定事項：一覧のページャは「前へ / 現在・総ページ / 次へ」形式、一覧カードの空項目は非表示（「-」を出さない）を正とする。
