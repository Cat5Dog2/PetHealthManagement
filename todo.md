...
## 追加タスク（必要に応じて）
- [ ] **第1段階UI刷新**：正式アプリ名「うちの子健康カルテ」への表示統一、共通レイアウトのモバイルファースト化、既存機能（Home/MyPage/Pets/HealthLogs/ScheduleItems/Visits/Account）のスマホ向けカード/フォームUI化
- [ ] **第1段階UIの範囲整理**：健康分析、月間カレンダー、通知設定、データバックアップ、利用規約、アプリについて等は新機能扱いとし、第1段階では未実装リンクやダミー画面を追加しない
- [ ] **ペットプロフィール項目の仕様反映**：`Sex` / `BirthDate` / `AdoptedDate` は正式仕様に残し、実装時に Entity / Migration / ViewModel / View / テストへ反映する
