...
- 対象：うちの子健康カルテ（ASP.NET Core MVC + Identity）
- バージョン：1.0
- 作成日：2026-01-13
- 作成者：Cat5Dog2
- 想定読者：開発者本人、レビュー担当者
...
> ユーザー向け表示名は「うちの子健康カルテ」を正とする。第1段階UI刷新では既存URL/APIの互換性を維持し、画面表示とナビゲーションのみをモバイルファーストに寄せる。
...
- 補足：体重（WeightKg）が2件以上ある場合、直近最大20件の**体重推移グラフ**（インラインSVG）を**1ページ目にのみ**表示する（2ページ目以降は体重クエリ自体を実行しない）。1ページ目でログはあるが体重が2件未満の場合は、グラフの代わりに案内文を表示する

#### GET `/HealthLogs/Record`
- 概要：ボトムナビ「記録」の遷移先。記録するペットを選ぶ記録ハブ
- 認可：認証必須（自分のペットのみ表示）
- 挙動：
  - 自分のペットが **0匹**：302 → `/Pets/Create?returnUrl=/MyPage`
  - 自分のペットが **1匹**：302 → `/HealthLogs/Create?petId={petId}`
  - 自分のペットが **2匹以上**：200（ペット選択画面のHTML。ペットごとに **健康ログ / 予定 / 通院** の作成リンクを表示）
