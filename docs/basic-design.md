...
- UI の色・レイアウトの細部、E2E テスト設計は対象外。ただし第1段階UI刷新では、正式アプリ名、モバイルファースト方針、既存機能に限定したナビゲーション方針を本書の正とする。
...
- POST 成功後は TempData（キー `AppStatusMessage`。Identity 管理画面の `StatusMessage` と衝突しない専用キー）による共通ステータスアラートで結果をフィードバックする。アラートは数秒後に自動で閉じる。
- OS のダークモード（`prefers-color-scheme: dark`）に追従した配色を提供する。
- CSP の `script-src` は `'self'` のみとし、確認ダイアログ・自動送信は `data-confirm` / `data-autosubmit` 属性 + `site.js` のイベントデリゲーションで実装する（inline handler 禁止）。
...
- 一覧上部（1ページ目のみ）に体重推移グラフ（インラインSVG）を表示：体重（`WeightKg`）2件以上で表示、直近最大20件。体重2件未満でログがある場合は案内文を表示
- **記録ペット選択（記録ハブ）：`/HealthLogs/Record`**。下部ナビ「記録」の遷移先。自分のペットが0匹ならペット登録へ、1匹なら健康ログ作成へ302、2匹以上で選択画面（ペットごとに健康ログ/予定/通院の作成リンク）を表示
