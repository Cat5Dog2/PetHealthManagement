...
| SCR-009-014 | ログイン済み（UserA、体重入りログ2件以上） | 1) 一覧を表示 | 一覧上部に体重推移グラフ（直近最大20件）が表示される | SS |
| SCR-009-015 | ログイン済み（UserA、体重入りログ1件以下） | 1) 一覧を表示 | 体重推移グラフは表示されない | SS |
...
| SCR-009-018 | ログイン済み（UserA、ペット2匹以上） | 1) ボトムナビ「記録」押下 | `/HealthLogs/Record` でペット選択画面が表示され、選択で Create へ遷移する | SS/NT |
...
| SCR-011-022 | ログイン済み（UserA） | 1) 保存成功後の遷移先を表示 | 「健康ログを保存しました。」等のステータスメッセージが表示される | SS |
