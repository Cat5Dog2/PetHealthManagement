# うちの子健康カルテ テストケース表（画面単位） v1.0

- 作成日：2026-01-17
- 対象：うちの子健康カルテ（Web / ASP.NET Core MVC）
...
| SCR-003-007 | ログイン済み（UserA） | 1) `/MyPage` を表示 | 「アカウント削除」の導線は表示されない（プロフィール編集画面へ移動済み） | SS |
...
| SCR-004-010 | ログイン済み（UserA） | 1) GET表示 | フォーム下部に「アカウントの削除」危険領域が表示され、「アカウント削除へ進む」で `/Account/Delete` に遷移する | SS |
| SCR-004-011 | ログイン済み（UserA） | 1) 表示名を変更して保存 → 2) 遷移先を表示 | 「プロフィールを更新しました。」のステータスメッセージが表示される | SS |
...
| SCR-009-013 | ログイン済み（UserA） | 1) 一覧の行「削除」押下 | 確認ダイアログが表示され、キャンセルで削除されない | SS/DB |
| SCR-009-014 | ログイン済み（UserA、体重入りログ2件以上） | 1) 一覧の1ページ目を表示 | 一覧上部に体重推移グラフ（直近最大20件）が表示される。2ページ目以降には表示されない | SS |
| SCR-009-015 | ログイン済み（UserA、体重入りログ1件以下でログあり） | 1) 一覧の1ページ目を表示 | グラフの代わりに「体重を2回以上記録すると〜」の案内文が表示される | SS |
| SCR-009-016 | ログイン済み（UserA、値が空の項目を含むログ） | 1) 一覧を表示 | 値が空の項目（体重/食事量/散歩/便/メモ）は行ごと非表示になり「-」は表示されない | SS |
| SCR-009-017 | ログイン済み（UserA、データ11件以上） | 1) ページャの「次へ」「前へ」で移動 | 前へ/次へ形式のページャで移動でき、端のページでは該当ボタンが無効表示になる | SS/NT |
| SCR-009-018 | ログイン済み（UserA、ペット2匹以上） | 1) ボトムナビ「記録」押下 | `/HealthLogs/Record` でペット選択画面が表示され、ペットごとの「健康ログ/予定/通院」から各 Create へ遷移できる | SS/NT |
| SCR-009-019 | ログイン済み（UserA、ペット1匹） | 1) ボトムナビ「記録」押下 | 選択画面を挟まず `/HealthLogs/Create?petId={petId}` へ302リダイレクトされる | NT |
| SCR-009-020 | ログイン済み（UserA、ペット0匹） | 1) ボトムナビ「記録」押下 | `/Pets/Create?returnUrl=/MyPage` へ302リダイレクトされる | NT |
...
| SCR-011-020 | ログイン済み（UserA） | 1) Create画面表示 | 「便の様子」はセレクトボックス（未選択/良好/普通/軟便/下痢/血便/便秘）で表示される | SS |
| SCR-011-021 | ログイン済み（UserA、選択肢外の既存値を持つログ） | 1) Edit画面表示 | 既存値が選択肢に含まれた状態で選択されており、保存しても値が失われない | SS/DB |
| SCR-011-022 | ログイン済み（UserA） | 1) 保存成功後の遷移先を表示 | 「健康ログを保存しました。」等のステータスメッセージが表示され、数秒後に自動で閉じる | SS |
| SCR-011-023 | ログイン済み（UserA） | 1) Identity のパスワード変更を成功させる | 変更画面内に「パスワードを変更しました。」が1回だけ表示される（共通アラートとの二重表示・誤スタイルにならない） | SS |
...
| SCR-012-018 | ログイン済み（UserA） | 1) typeFilterで絞り込み（Vaccine等） | 該当種別のみ表示され、その他の条件は保持される | SS/NT |
| SCR-012-019 | ログイン済み（UserA） | 1) typeFilterのセレクトを変更 | 検索ボタンなしで自動送信され、絞り込み結果が表示される | SS/NT |
| SCR-012-020 | ログイン済み（UserA） | 1) 「完了にする」のラベル文字をタップ | チェックボックスが反応し、完了トグルが送信される（ラベルとinputが関連付いている） | SS/NT |
| SCR-012-021 | ログイン済み（UserA） | 1) 行の「削除」押下 | 確認ダイアログが表示され、キャンセルで削除されない | SS/DB |
