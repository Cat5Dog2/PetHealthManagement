...
### 1.1 共通レイアウト／ナビ
- 第1段階UI刷新では、スマホ幅を主対象にしたモバイルファーストUIを正とする。
- 認証済み画面では、上部ヘッダに加えて下部ナビを採用してよい。ただし遷移先は既存機能に限定し、未実装の健康分析/月間カレンダー/通知設定/バックアップ等へのリンクは置かない。
- ユーザー向けアプリ名は **「うちの子健康カルテ」** を表示する。

| 項目ID | 項目名 | 種別 | 表示条件 | 動作/遷移 |
|---|---|---|---|---|
| COM-01 | Logo（うちの子健康カルテ） | リンク | 常時 | `/` |
| COM-02 | ホーム（MyPage） | リンク/下部ナビ | 認証時 | `/MyPage` |
...
- URL：`/MyPage`（認証必須）
- ユーザー向けUIでは「ホーム」または「設定」相当として表現してよい。ただし URL と認可仕様は `/MyPage` のまま維持する。
...
| 003-09 | 設定メニュー | D/A | リンク一覧 |  |  | 第1段階では既存のプロフィール編集/パスワード変更/ペット登録に限定 |
...
| 004-A03 | アカウント削除へ進む | A | リンク |  |  |  | `/Account/Delete`。注意書き付きの危険領域としてフォーム下部に表示（SCR-003 から移動） |
...
| 011-02 | 体重(kg) | I | 数値 | 任意 | 0.0〜200.0 | `inputmode="decimal"` で小数キーボードを表示 |
| 011-03 | 食事量(g) | I | 数値 | 任意 | 0〜5000 | |
| 011-04 | 活動(分) | I | 数値 | 任意 | 0〜1440 | |
| 011-05 | 排せつ状態 | I | セレクト | 任意 | Max 50 | 選択肢：未選択/良好/普通/軟便/下痢/血便/便秘。編集時に選択肢外の既存値はそのまま選択肢に含めて保持 |
...
| 項目ID | 項目名 | I/D/A | 種別 | 必須 | 送信名（クエリ） | 制約/形式 | 参照元 | 備考 |
|---|---|---:|---|---:|---|---|---|---|
| 012-F01 | 種別フィルタ | I | セレクト |  | `typeFilter` |  |  | 変更時に自動送信（JS無効時のみ `noscript` の絞り込みボタンを表示） |
...
| 014-02 | 種別（ItemType） | I | セレクト | ✓ | 固定推奨：Vaccine/Medicine/Visit/Other、Max 20 | |
