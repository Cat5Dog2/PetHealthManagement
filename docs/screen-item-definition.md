...
| 011-A01 | 保存 | A | ボタン |  |  | POST後：`returnUrl` 優先、無効なら（作成）一覧へ／（編集）詳細 or 一覧へ |
| 011-A02 | キャンセル | A | リンク |  |  | `returnUrl` 優先、無効なら一覧へ |
...
| 014-A01 | 保存 | A | ボタン |  |  | POST後：`returnUrl` 優先、無効なら一覧へ |
| 014-A02 | キャンセル | A | リンク |  |  | `returnUrl` 優先、無効なら一覧へ |
...
| 017-A01 | 保存 | A | ボタン |  |  | POST後：`returnUrl` 優先、無効なら（作成）一覧へ／（編集）詳細 or 一覧へ |
| 017-A02 | キャンセル | A | リンク |  |  | `returnUrl` 優先、無効なら一覧へ |
